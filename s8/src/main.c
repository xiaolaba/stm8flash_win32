...
// REPLY-MODE
	...
	if(dtr_reset)
	{
		...
		//usleep(10000); //FIXME, 10ms
		sleep(10); // 10ms, for windows
	}
...
void show_help(char *name) {
	fprintf(stderr,
		"Usage: %s [-bvngfhc] [-[rw] filename] serial-port (linux /dev/ttyS0, Windows COM6)\n"
		...
		"		%s -w filename -v -g 0x0 serial-port\n"
		"\n"
		"	Read flash to file:\n"
		"		%s -r filename serial-port\n"
		"\n"
		"	Start execution:\n"
		"		%s -g 0x0 serial-port\n",
		name,
