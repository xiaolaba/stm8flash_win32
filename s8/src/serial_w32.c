...
/* compiler warning !!
gcc -c -static -g -Wall -fPIC  -DLANTRONIX_CPM -I/include -I/user/lantronix/libcp -I./parsers -I. serial_platform.c -o serial_platform.o
In file included from serial_platform.c:2:0:
serial_w32.c: In function 'serial_open':
serial_w32.c:61:11: warning: assignment discards 'const' qualifier from pointer target type [-Wdiscarded-qualifiers]
   devName = device;
           ^
*/
...
*/
	...
	// compiler warning fixed, 2025-05-21, xiaolaba. minGW/win10
	char *devName;
	...
	if (strlen(device) > 4 && device[0] != '\\') {
		...
		devName = calloc(1, strlen(device) + 1);
...
serial_err_t serial_dtr_reset(serial_t *h) {
    assert(h && h->fd != INVALID_HANDLE_VALUE);

    // Set DTR (low to reset for Arduino-style)
    if (!EscapeCommFunction(h->fd, CLRDTR)) {
        return SERIAL_ERR_SYSTEM;
    }

    // Wait for 10ms (Arduino-style reset typically requires a short pulse)
    Sleep(10);

    // Clear DTR (set it back high)
    if (!EscapeCommFunction(h->fd, SETDTR)) {
        return SERIAL_ERR_SYSTEM;
    }
